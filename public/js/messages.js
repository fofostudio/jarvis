...
(function ($) {
    ...
    $(document).on("click", "#button-reply-msg", function (s) {
        ...
        var dataSent =
            message_user == 1
                ? '<i class="far fa-paper-plane"> pay <span class="pay_amount">' +
                  text_message_charge +
                  "</span> " +
                  wallet_symbol +
                  "</i>"
                        ...
                        success: function (result) {
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    $(".balanceWallet").text(result.wallet);
                                }
                            }
                            ...
                            if (result.success != false) {
                                ...
                                //$('.pay_amount').text(text_message_charge);
            ...
            request = $.ajax({
                ...
                if (res) {
                    ...
                    if (res.userOnline) {
                        ...
                        $(".user-status")
                            .removeClass("user-offline")
                            .addClass("user-online");
                    } else if (res.userOnline == false) {
                        $(".user-status")
                            .removeClass("user-online")
                            .addClass("user-offline");

                        $("#lastSeen").remove();

                        $("#timeAgo").html(
                            '<small class="timeAgo" id="lastSeen" data="' +
                                res.last_seen +
                                '" style="display: inline;"></small>',
                        );
    ...
    $(document).on("click", ".loadMoreMessages", function (e) {
        ...
        wrapContainer.html(
            '<span class="spinner-border align-middle text-primary mb-2"></span>',
        );

        $.ajax({
            url:
                URL_BASE +
                "/loadmore/messages?id=" +
                user_id +
                "&skip=" +
                allElements,
        })
