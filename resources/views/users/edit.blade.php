...
@section('content')
    <div class="container">
        <h2 class="mb-4">{{ __('admin.edit_operator') }}</h2>
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('neighborhood') is-invalid @enderror"
                                    ...
                                    value="{{ old('neighborhood', $user->neighborhood) }}" required>
                                @error('neighborhood')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="password" class="form-control" id="password" value="********"
                                        readonly>
                                    <button type="button" class="btn btn-warning" id="reset-password"
...
@section('javascripts')
    <script>
        document.getElementById('reset-password').addEventListener('click', function() {
            if (confirm("{{ __('admin.confirm_reset_password') }}")) {
                ...
                fetch(`/users/${userId}/reset-password`, {
                        ...
                        body: JSON.stringify({
                            password: 'TeamSic2024'
                        })
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            alert("{{ __('admin.password_reset_success') }}");
                        } else {
                            alert("{{ __('admin.password_reset_error') }}");
                        }
                    })
                    .catch(error => {
