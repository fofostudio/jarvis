...
@section('content')
    <h5 class="mb-4 fw-light">
        ...
        <i class="bi-chevron-right me-1 fs-6"></i>
        <span class="text-muted">{{ __('admin.edit_operator') }}</span>
    </h5>

    <div class="content">
        <div class="card shadow-custom border-0">
            <div class="card-body p-lg-5">
                <form method="post" action="{{ url('settings/operators/' . $user) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="mb-3 text-center">
                                ...
                                <div class="avatar-preview rounded-circle mb-3" style="background-image: url('{{ $user->avatar ? asset('storage/' . $user->avatar) : asset('images/placeholder.png') }}')"></div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('jornada') is-invalid @enderror" id="jornada" name="jornada" required>
                                    ...
                                    <option value="morning" {{ old('jornada', $user->jornada) == 'morning' ? 'selected' : '' }}>{{ __('admin.morning') }}</option>
                                    <option value="afternoon" {{ old('jornada', $user->jornada) == 'afternoon' ? 'selected' : '' }}>{{ __('admin.afternoon') }}</option>
                                    <option value="night" {{ old('jornada', $user->jornada) == 'night' ? 'selected' : '' }}>{{ __('admin.night') }}</option>
                                </select>
                                @error('jornada')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('grupo_id') is-invalid @enderror" id="grupo_id" name="grupo_id" required>
                                    ...
                                    @foreach($groups as $group)
                                        <option value="{{ $group->id }}" {{ old('grupo_id', $user->grupo_id) == $group->id ? 'selected' : '' }}>{{ $group->nombre_grupo }}</option>
                                    @endforeach
                                </select>
                                @error('grupo_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('identification') is-invalid @enderror" id="identification" name="identification" value="{{ old('identification', $user->identification) }}">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('neighborhood') is-invalid @enderror" id="neighborhood" name="neighborhood" value="{{ old('neighborhood', $user->neighborhood) }}">
                                @error('neighborhood')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <button type="submit" class="btn btn-sm btn-dark float-lg-end mt-1 mt-lg-0">{{ __('admin.update') }}</button>
                </form>
            </div>
        </div>
    </div>
