...
@section('content')
    <h5 class="mb-4 fw-light">
        ...
        <span class="text-muted">{{ __('admin.platforms') }} ({{$platforms->count()}})</span>
                            ...
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="active">{{ trans('admin.url') }}</th>
                                        <th class="active">{{ trans('admin.access_mode') }}</th>
                                ...
                                <tbody>
                                    @forelse ($platforms as $platform)
                                        <tr>
                                            <td>{{ $platform->name }}</td>
                                            <td>{{ $platform->url }}</td>
                                            <td>{{ $platform->access_mode }}</td>
                                            <td>
                                                <a href="{{ route('platforms.edit', $platform) }}" class="btn btn-success rounded-pill btn-sm me-2">
                                                    <i class="bi-pencil"></i>
                                                </a>
                                                <form action="{{ route('platforms.destroy', $platform) }}" method="POST" class="d-inline-block">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center">{{ trans('admin.no_platforms_found') }}</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
